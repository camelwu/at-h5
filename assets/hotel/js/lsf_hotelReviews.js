...
(function() {
  ...
  function V(data) {
    ...
    console.log(data);
    var str1 = '<section>' + '<p>设施<i>' + data[0].reviewRatingsList[0].scoringScaleID + '</i>分</p>' + '<p class="lsf_gra_p2">客房<i>' + data[0].reviewRatingsList[1].scoringScaleID + '</i>分</p>' + '</section>' + '<section>' + '<p>地点<i>' + data[0].reviewRatingsList[3].scoringScaleID + '</i>分</p>' + '<p class="lsf_gra_p2">服务<i>' + data[0].reviewRatingsList[4].scoringScaleID + '</i>分</p>' + '</section>' + '<section class="last_sec">' + '<p>清洁度<i>' + data[0].reviewRatingsList[2].scoringScaleID + '</i>分</p>' + '<p class="lsf_gra_p2">物有值<i>' + data[0].reviewRatingsList[5].scoringScaleID + '</i>分</p>' + '</section';
    var str2 = '';
    //评论分页设置
    function num_show(start, end) {
      ...
      if (myEnd < comments.length) {
        for (var i = myStart; i < myEnd; i++) {
          ...
          for (var j = 0; j < 5; j++) {
            if ((j + 1) <= star) {
              str3 += '<li class="fl" style="background:url(../images/ui/icons1.png) -0.08rem -3.9rem;;background-size:9.8rem 3rem;"></li>';
        ...
        for (var i = myStart; i < comments.length; i++) {
          ...
          for (var j = 0; j < 5; j++) {
            if ((j + 1) <= star) {
              ...
              str3 += '<li class="fl" style="background:url(../images/ui/icons1.png) 9.46rem -3.9rem;background-size:9.8rem 3rem;"></li>';
            } else {
              str3 += '<li class="fl" style="background:url(../images/ui/icons1.png) -9.6rem -3.9rem;background-size:9.8rem 3rem;"></li>';
            }
          }
