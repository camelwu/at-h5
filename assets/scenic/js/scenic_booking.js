  ...
  var Command = function () {
    var Adapter = {
      initRedbag: function () {
        ...
        if (memberid) {
          //订单总额
          var orderAmount = Adapter.totalPrice();
          //$("#coupon").show();
          RedBag.init(memberid, "scenic", "redBagWrap", "coupon", orderAmount);

          //更新订单详情
          var callbacks = $.Callbacks();
          callbacks.add(Adapter.updatePriceDetail);
      ...
      render: function (data) {
        ...
        ExtendData.DefaultDate = Adapter.formatDate({
          date: ExtendData.DetailData.data.defaultDepartStartDate,
          format: "yyyy-MM-dd"
        });
        ExtendData.StartDate = Adapter.formatDate({
          date: ExtendData.DetailData.data.departValidFrom,
          format: "yyyy-MM-dd"
        });
      ...
      updatePriceDetail: function () {
        ...
        if ($(".js_booking_package_pre_adult_num").html() !== undefined) {
          for (var i = 0; i < ExtendData.SearchPriceData.data.prices.length; i++) {
            if (ExtendData.SearchPriceData.data.prices[i].category == "ADULT") {
              ...
              outString = ejs.render(tplString, {
                tourCategory: "成人票",
        ...
        if(couponEle.length>0 && couponEle.attr("data-code") && couponEle.css("display")!== 'none'){
          var amount = couponEle.attr("data-amount");
          var container = $(".js_booking_footer_popprice_list");
          var newPrice = Adapter.totalPrice() -amount;
          $(".js_booking_package_totalprice").html(newPrice);
          var couponsInfo = '<li class="booking_footer_popprice_item red_info">'+
            '<div class="booking_footer_popprice_item_l">红包</div>' +
            '<div class="booking_footer_popprice_item_r">-¥' + amount + '</div>' +
            '</li>';
      ...
      validateBookingPackage: function () {
        ...
        if(couponEle.length>0 && couponEle.attr("data-code") && couponEle.css("display")!== 'none') {
          var vouchers = [], amount = [];
          vouchers.push(couponEle.attr("data-code"));
          OrderParam.Parameters.Vouchers = vouchers;
        }
          //成人数量
        ...
        if (isNaN(childCount) || childCount == 0) {
          ...
          for (var i = 0; i < childCount; i++) {
            ...
            //验证儿童年龄input是否为空
            if (!T.Command().execCommand({
                command: "validate",
                param: {type: "text", data: childTmp, rules: "required", tips: "儿童年龄不能为空"}
              })) {
              return false;
            }
            //验证儿童年龄范围
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "tourweek", data: "", rules: "", tips: "不开放,请重新选择日期"}
          })) {
          return false;
        }
        //创建订单参数 景点信息
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "text", data: ticketLastName, rules: "required|en", tips: "取票人信息姓不能为空|请您输入英文的取票人姓名"}
          })) {
          return false;
        }
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "text", data: ticketFirstName, rules: "required|en", tips: "取票人信息名不能为空|请您输入英文的取票人姓名"}
          })) {
          return false;
        }
        //取国籍信息
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "text", data: NationalityCode, rules: "required", tips: "国籍信息不能为空"}
          })) {
          return false;
        }
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "text", data: contackLastName, rules: "required|en", tips: "联系人信息姓不能为空|请您输入英文的联系人姓名"}
          })) {
          return false;
        }
        //验证联系人信息
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "text", data: contactFirstName, rules: "required|en", tips: "联系人信息名不能为空|请您输入英文的联系人姓名"}
          })) {
          return false;
        }
        //取区号信息
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "text", data: CountryCode, rules: "required", tips: "电话区号信息不能为空"}
          })) {
          return false;
        }
        ...
        if (!T.Command().execCommand({
            ...
            param: {
              type: "phone",
              data: phone,
              extenddata: CountryCode,
              rules: "required|phone|CN",
              tips: "联系电话不能为空|联系电话格式不正确|联系电话格式不正确"
            }
          })) {
          return false;
        }
        ...
        if (!T.Command().execCommand({
            command: "validate",
            param: {type: "email", data: email, rules: "required|email", tips: "邮箱不能为空|请输入正确邮箱格式"}
          })) {
          return false;
        }
        //联系人信息
        ...
        if (Pickup) {
          ...
          if (!T.Command().execCommand({
              command: "validate",
              param: {type: "text", data: PickupID, rules: "required", tips: "接送信息不能为空"}
            })) {
            return false;
          }
      ...
      if (json.success) {
        ...
        $("#js_pickup_pop_suggest_list").click(function (e) {
          ...
          if (tar.nodeName.toLowerCase() === 'li') {
            ...
            var value = $(tar).html();
            $(".js_booking_package_pickup_selecttext").attr("data-key", id).html(value);
            $(".js_booking_package_pickup_selecttext").addClass("current");
