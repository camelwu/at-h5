...
var footer = (function() {
  ...
  //遮罩层容器
    ...
    // maskerSc
    maskerSc : function(id, c) {
      if (!maskerSc) {
        maskerSc = document.createElement('div');
        id ? maskerSc.id = id : null;
        c ? maskerSc.className = c : null;
        maskerSc.style.display = "none";
        if (!id && !c) {
          maskerSc.style.position = 'fixed';
          maskerSc.style.left = 0;
          maskerSc.style.top = 0;
          maskerSc.style.width = '100%';
          maskerSc.style.height = '100%';
          maskerSc.style.background = 'rgba(0,0,0,0.6)';
          maskerSc.style.zIndex = 20;
        }
        document.body.appendChild(masker);
        return this;
      } else {
