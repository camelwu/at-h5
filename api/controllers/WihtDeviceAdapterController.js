...
//各业务类型订单Order-Code
var apiOrderDic={
  "10100006":{tpye:"H",name:"酒店",apiName:"酒店生成单接口"},
  "20100007":{tpye:"T",name:"景点",apiName:"景点生成单接口"},
  "30100002":{tpye:"F",name:"机票",apiName:"机票生成单接口"},
  "50100004":{tpye:"FH",name:"机酒",apiName:"机酒生成单接口"},
  "40100004":{tpye:"HT",name:"酒景",apiName:"酒景生成单接口"},
  "60100010":{tpye:"FHT",name:"机酒景" ,apiName:"机酒景生成单接口"},
}
...
module.exports = {
  apiWihtDeviceID: function (req, res) {
    ...
    var bodyString = JSON.stringify(getWithTrickParam(req,res,postJson));
    ...
    if (headers.flag == 1) {
      ...
      request(_api, option, function (err, httpResponse, body) {
        ...
        if(parseInt(spendTime) > 5000){
          console.log(bodyString);
          console.log(new Date() + "--apiWihtDevice 接口访问时间：" + (requestEndTime-reqeustStartTime));
        }
